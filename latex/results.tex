...
\newpage
\section{Mass Flow Rate Experimental Results}
\subsection{Inlet Pressure of 4 bar}
\begin{figure}[htbp]
    \centering
...
\subsection{Inlet Pressure of 5 bar}
\begin{figure}[htbp]
    \centering
...
\subsection{Inlet Pressure of 6 bar}
\begin{figure}[htbp]
    \centering
...
\section{Mass Flow Rate Discussion}
For each of the tests, the raw load cell readings can be seen on the first graph. Most graphs show that there was noise in the readings resulting in a few data points being drastically larger or smaller than the surrounding data. While steps were taken to prevent this like taping down the screw terminal incase the problem was from faulty wire connections but this never fixed the issue. The fact that the noise is less common in the lower pressure recordings implies that this could be somehow caused by electrostatic charge buildup in the sand from the triboelectric effect.

These anomolous
The first thing to note from the tests is the large variation between the shapes of the mass flow rate even within tests of the same inlet pressure. Comparing the mass flow rate for all tests with a 4 bar pressure inlet shows this phenomenon. An important thing to note is due to the smoothing applied, done by taking the average of a 100 sample window that moves through the data, there is edge distortion at the start and end. The sensors were polled at a frequency of 10Hz meaning up to 10s of starting flow will be averaged with the behaviour before the valves were opened. Accounting for this by looking at the behavour 10s after the valves were opened and 10s before the tank is empty, still highlights the variability. A difference of almost 0.001kg/s of mass flow rate between tests 1 and 2 compared to test 3 for a pressure inlet of 4 bar.

This was initially hypothesised to be because the low pressure means that the system is in a transitional phase between a gas-solid choked and unchoked regime. If this were to be the case, then the subesquent tests at higher pressures should show a more consistent flow rate. While this is seen in testing with a 5 bar inlet pressure, with most of the flow sitting between 0.003 and 0.004kg/s
